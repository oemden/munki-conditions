...
#
# Writes Names conditions : CADTools presence for reinstall
...
for CC_VERSION in "${CC_VERSIONS[@]}"
 do
   ...
   ILLUSTRATOR_CC_CADTOOLS_PLUGIN_PATH="/Applications/${ILLUSTRATOR_CC_DIRNAME} ${CC_VERSION}/Plug-ins.localized/CADtools.aip"
   ILLUSTRATOR_CC_APP="/Applications/${ILLUSTRATOR_CC_DIRNAME} ${CC_VERSION}/Adobe Illustrator.app"

  echo "---- CADTools plugin Check for ${ILLUSTRATOR_CC_APP} Starting ----"

if [[ -e "${ILLUSTRATOR_CC_APP}" ]] ; then #01 (illustrator app)

 echo " Illustrator CC ${CC_VERSION} is installed" ; echo
 
 extraecho "ILLUSTRATOR_CC_CADTOOLS_PLUGIN_PATH: ${ILLUSTRATOR_CC_CADTOOLS_PLUGIN_PATH}"
 extraecho "ILLUSTRATOR_CC_APP: ${ILLUSTRATOR_CC_APP}"
 
  if [[ ! -d "${ILLUSTRATOR_CC_CADTOOLS_PLUGIN_PATH}" ]] ; then #(cadtools not installed)
   echo
   echo " CADTOOLS ${CADTOOL_VERSION} plugin is not installed for Illustrator CC ${CC_VERSION}"
   echo "Needs to be installed" ; echo
   installCadtools="TRUE"
   extraecho "installCadtools: ${installCadtools}"
  fi
else
...
if [[ "${installCadtools}" == "TRUE" ]] ; then #02 (cadtools installed)
   echo " CADTOOLS plugin is missing for some version of Illustrator"
   "${DEFAULTS}" write "${COND_DOMAIN}" "INSTALLCADTOOLS" TRUE
fi
